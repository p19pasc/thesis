<?php
    // Create a session with archiveDataSUBMIT.php to pass message from archiveDataINSERT.php through $_SESSION superglobal
    session_start();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if(isset($_POST["submit_archiveData"])){
            // Create the INSERT INTO query until before "VALUES" 
            $insert_string = "INSERT INTO archivedata(";
            ...
            foreach($_POST as $name => $value){
                // Each variable that has value from the user in the form add it to the query
                    if(!empty($value)){
                        ...
                        if($name != 'archiveSourcesID' && $name != 'documentEditionYear' && $name != 'documentReferenceYear' && $name != 'documentReferenceYearEnd' && $name != 'hasStatistics' && $name != 'documentCategoryID' && $name !='informationCategoryID' && $name != 'placeOfRecord' && $name != 'placeOfReference'){
                            // Escape ', ", \, and null 
            ...
            // Initialize the $_SESSION["message"] with the message that will be displayed to the user in archiveDataSUBMIT.php
