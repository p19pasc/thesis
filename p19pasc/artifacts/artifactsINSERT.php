<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if(isset($_POST["submit_artifacts"])){
            // Create the INSERT INTO query until before "VALUES" 
            $insert_string = "INSERT INTO artifacts(";
            ...
            foreach($_POST as $name => $value){
                // Each variable that has value from the user in the form add it to the query
                    if(!empty($value)){
                        ...
                        if($name != 'storagePlaceID' && $name != 'artifacttypeID' && $name != 'siteID' && $name != 'materialID' && $name != 'periodID' && $name != 'wholeObject'){
                            // Escape ', ", \, and null 
